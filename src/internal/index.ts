...
export const genLoadingState =
  <R>(cache: RxlCache<R>) =>
  (): RxlAsyncState<R | undefined> => ({
    isLoading: true,
    error: null,
    data: cache.data,
  });
...
export const handleUpdate =
  <R>(cache: RxlCache<R>, error$: Subject<unknown>) =>
  (obs: Observable<R | undefined>): Observable<RxlAsyncState<R | undefined>> =>
    obs.pipe(
